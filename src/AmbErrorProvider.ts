...
export async function errorByTxHash(txHash: string, provider: ethers.providers.Provider) {
  const tx = await provider.getTransaction(txHash);
  if (!tx) throw new Error(`Transaction ${txHash} not found`);

  return await provider.call(
    { data: tx.data, value: tx.value, from: tx.from, to: tx.to, gasLimit: tx.gasLimit, gasPrice: tx.gasPrice },
    tx.blockNumber
  );
}
